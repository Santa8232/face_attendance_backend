...
const dailySummary = asyncHandler(async (req, res) => {
  ...
  const query = { attendance_date: targetDate };
  if (offId) query.office_id = offId;

  const logs = await store.findMany(TABLES.ATTENDANCE_LOGS, query);

  const empQuery = { is_active: true };
  if (offId) empQuery.office_id = offId;
  const employees = await store.findMany(TABLES.EMPLOYEES, empQuery);
...
const lateArrivals = asyncHandler(async (req, res) => {
  ...
  const query = { event_type: "CHECK_IN" };
  if (offId) query.office_id = offId;

  let logs = await store.findMany(TABLES.ATTENDANCE_LOGS, query);

  // Filter by month and shift_status in-memory as store.js doesn't support LIKE or complex where yet
  logs = logs.filter(
    (l) =>
...
const monthlyExport = asyncHandler(async (req, res) => {
  ...
  // Fetch all summaries for the month
  let summaries = await store.getAll(TABLES.ATTENDANCE_SUMMARY);
  summaries = summaries.filter(
    (s) => String(s.attendance_date).startsWith(month) && empIds.has(s.employee_id),
  );
