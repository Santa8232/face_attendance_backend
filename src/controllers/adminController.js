...
const listDepartments = asyncHandler(async (req, res) => {
  ...
  const depts = await store.findMany(TABLES.DEPARTMENTS, query);
  return ok(res, depts);
});
...
const listShifts = asyncHandler(async (req, res) => {
  ...
  const query = {};
  if (office_id) query.office_id = parseInt(office_id);
  const shifts = await store.findMany(TABLES.SHIFTS, query);
...
const assignShift = asyncHandler(async (req, res) => {
  ...
  return ok(
    res,
    { shift_id: shift.id, assigned_to: updated, count: updated.length },
    "Shift assigned",
  );
});
...
const listGeofences = asyncHandler(async (req, res) => {
  ...
  const query = {};
  if (office_id) query.office_id = parseInt(office_id);
  const geos = await store.findMany(TABLES.GEOFENCES, query);
  return ok(res, geos);
});
...
const validateGeofence = asyncHandler(async (req, res) => {
  ...
  if (!geos.length)
    return fail(res, "No active geofence found for this office", 404);
...
const getPolicyByOffice = asyncHandler(async (req, res) => {
  const policy = await store.findOne(TABLES.ATTENDANCE_POLICIES, {
    office_id: parseInt(req.params.officeId),
  });
...
const listExceptions = asyncHandler(async (req, res) => {
  ...
  const excs = await store.findMany(TABLES.ATTENDANCE_EXCEPTIONS, query);
  return ok(res, excs);
});

const raiseException = asyncHandler(async (req, res) => {
  const { employee_id, exception_type, description, attendance_id } = req.body;
  if (!employee_id || !exception_type)
    return fail(res, "employee_id and exception_type are required");
...
const listAuditLogs = asyncHandler(async (req, res) => {
  ...
  const query = {};
  if (actor_user_id) query.actor_user_id = parseInt(actor_user_id);
  if (action_type) query.action_type = action_type;

  let logs = await store.findMany(TABLES.AUDIT_LOGS, query);
